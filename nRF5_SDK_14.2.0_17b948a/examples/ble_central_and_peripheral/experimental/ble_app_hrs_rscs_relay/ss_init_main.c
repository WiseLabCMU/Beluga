...
SemaphoreHandle_t tx_wait, rx_wait,to_wait,err_wait;
...
int ss_init_run(void)
{
  ...
  * set by dwt_setrxaftertxdelay() has elapsed. */
  ...
  while (!(tx_int_flag))
  {}; //Here is a wait that occurs after transmit is started, waiting for tx to end
  ...
  //if( (uxQueueMessagesWaiting((QueueHandle_t)rx_wait) || uxQueueMessagesWaiting((QueueHandle_t)to_wait))){
  //if (((xSemaphoreTake(rx_wait,10) == pdTRUE ) || (xSemaphoreTake(to_wait,10) == pdTRUE))){
...
void ss_initiator_task_function (void * pvParameter)
{
  ...
  tx_wait = xSemaphoreCreateBinary();
  rx_wait = xSemaphoreCreateBinary();
  to_wait = xSemaphoreCreateBinary();
  err_wait = xSemaphoreCreateBinary();
