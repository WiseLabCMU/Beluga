...
static struct anchor anchorList[NUM_ANCHORS];
 */
  ...
  * set by dwt_setrxaftertxdelay() has elapsed. */
  ...
  {		
    ...
    if (frame_len <= RX_BUF_LEN)
    {
      ...
      uint8 posX = rx_buffer[18];
      uint8 posY = rx_buffer[19];
      uint8 posZ = rx_buffer[20];
      printf("X: %d\n", posX);
      printf("Y: %d\n", posY);
      printf("Z: %d\n", posZ);
      /*
      uint8 unit = rx_buffer[21];
      
      anchorList[unit-1] = anc;
      anc.posX = posX;
      anc.posY = posY;
      anc.posZ = posZ;
      anc.num = unit;
